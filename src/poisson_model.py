...
def fit_attack_defence(matches_df: pd.DataFrame, l2_reg: int = 0.02):
    ...
    def nll(params):
        # vector of attack, defence, home_adv.
    ...
    x0 = np.concatenate([np.zeros(n_teams), np.zeros(n_teams), [0.065]])
    # Walks until nll's gradient is zero.
    result = optimize.minimize(nll, x0, method='L-BFGS-B')  # quasi-Newton method, uses the gradient of log likelihood.
    ...
    return teams, attack, defence, home_adv
